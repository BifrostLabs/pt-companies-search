...
class APIKeyRotator:
    ...
    def rotate_key(self) -> str:
        ...
        while attempts < len(self.keys):
            ...
            if self.key_status[key]['available']:
                ...
                print(f"🔄 Rotated from Key #{old_index + 1} to Key #{self.current_index + 1}")
                return key
            ...
            if attempts >= len(self.keys):
                print("⚠️  All keys rate limited. Waiting 65 seconds for reset...")
                time.sleep(65)
    ...
    def mark_key_limited(self, key: str, retry_after: int = 60):
        ...
        print(f"⏳ Key #{idx} rate limited. Marking as unavailable.")
    ...
    def update_credits(self, key: str, credits: Dict):
        ...
        if credits:
            ...
            if month == 0 or day == 0:
                self.key_status[key]['available'] = False
                idx = self.key_status[key]['index'] + 1
                print(f"❌ Key #{idx} has insufficient credits (M:{month}, D:{day})")
