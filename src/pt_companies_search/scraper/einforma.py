...
def fetch_new_companies(page: int = 1) -> List[Dict[str, Any]]:
    """Fetch the list of new companies from eInforma.pt (single page)"""
    base_url = "https://www.einforma.pt/novas-empresas-portuguesas"
    url = base_url if page == 1 else f"{base_url}?paginaActual={page}"
...
def fetch_all_companies(max_pages: int = 5, delay: float = 2.0) -> List[Dict[str, Any]]:
    """Fetch companies across multiple pages, deduplicating by NIF"""
    import time
    all_companies: Dict[str, Dict[str, Any]] = {}

    for page in range(1, max_pages + 1):
        try:
            companies = fetch_new_companies(page=page)
            if not companies:
                break  # No more results

            new_on_page = 0
            for c in companies:
                if c["nif"] not in all_companies:
                    all_companies[c["nif"]] = c
                    new_on_page += 1

            if new_on_page == 0:
                break  # Duplicate page — stop pagination

            if page < max_pages:
                time.sleep(delay)
        except Exception:
            break  # Stop on any error but return what we have
